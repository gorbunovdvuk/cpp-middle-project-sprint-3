...
inline auto YearBetween(int year_left, int year_right) {
    return [year_left, year_right](const Book& book) {
        return year_left <= book.year && book.year <= year_right;
    };
}
...
auto all_of(Predicate&&... predicate) {
    return [...predicate = std::forward<Predicate>(predicate)](const Book& book) {
...
auto filterBooks(Iterator begin, Sentinel end, Predicate predicate) {
    ...
    std::ranges::copy(
        std::ranges::subrange(begin, end) | std::views::filter(predicate),
