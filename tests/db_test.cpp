...
TEST(StatisticsTest, GetTopNBy) {
    ...
    };
    {
        ...
        EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Agatha Christie\", \"The Murder of Roger Ackroyd\", 1926, Genre::Mystery, 89, 220}");
        EXPECT_EQ(std::format<const Book&>("{}", result[1]), "{\"Agatha Christie\", \"Death on the Nile\", 1937, Genre::Mystery, 88, 210}");
    }
    {
        auto result = getTopNBy(books, 2, comp::GreaterByRating{});
        EXPECT_EQ(result.size(), 2);
        EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Haruki Murakami\", \"1Q84\", 2009, Genre::Fiction, 85, 150}");
        EXPECT_EQ(std::format<const Book&>("{}", result[1]), "{\"Isaac Asimov\", \"The Caves of Steel\", 1953, Genre::SciFi, 87, 120}");
    }
}
...
TEST(StatisticsTest, SampleRandomBook) {
    ...
    EXPECT_EQ(result.size(), 2);
    EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Jane Austen\", \"Pride and Prejudice\", 1813, Genre::Fiction, 94, 300}");
    EXPECT_EQ(std::format<const Book&>("{}", result[1]), "{\"Isaac Asimov\", \"The Caves of Steel\", 1953, Genre::SciFi, 87, 120}");
}

constexpr BookDatabase<> bigBookDatabase() {
    return BookDatabase<>{
        {"Crime and Punishment", "Fyodor Dostoevsky", 1866, Genre::Fiction, 95, 250},
...
TEST(StatisticsTest, YearBetweenTest) {
    ...
    EXPECT_EQ(result.size(), 2);
    EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Stephen Hawking\", \"A Brief History of Time\", 1988, Genre::NonFiction, 93, 340}");
    EXPECT_EQ(std::format<const Book&>("{}", result[1]), "{\"Haruki Murakami\", \"Norwegian Wood\", 1987, Genre::Fiction, 88, 200}");
}
...
TEST(StatisticsTest, RatingAbove) {
    ...
    EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Fyodor Dostoevsky\", \"The Brothers Karamazov\", 1880, Genre::Fiction, 96, 220}");
}

TEST(StatisticsTest, AllOfTest) {
    auto books = bigBookDatabase();
    auto result = filterBooks(books.begin(), books.end(), all_of(
        RatingAbove(95),
        AuthorIs("Fyodor Dostoevsky"),
...
TEST(StatisticsTest, AnyOfTest) {
    auto books = bigBookDatabase();
    auto result = filterBooks(books.begin(), books.end(), any_of(
        RatingAbove(95),
        AuthorIs("Walter Isaacson")
    ));
    EXPECT_EQ(result.size(), 2);
    EXPECT_EQ(std::format<const Book&>("{}", result[0]), "{\"Fyodor Dostoevsky\", \"The Brothers Karamazov\", 1880, Genre::Fiction, 96, 220}");
    EXPECT_EQ(std::format<const Book&>("{}", result[1]), "{\"Walter Isaacson\", \"Steve Jobs\", 2011, Genre::Biography, 89, 160}");
}
